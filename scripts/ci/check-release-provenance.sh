...
# least one exists — with ONE exception: check runs produced by `.github/workflows/release-hygiene.yml`
# are ignored. A run still in flight is evidence neither way: it reports `conclusion: null`, and
# publish.yml's own job is always one of them.
#
# WHY THE EXCEPTION. release-hygiene.yml runs on `push: main`, and a release commit reaches main by a
# push like any other — so its checks run on the very commit about to be tagged, before the tag
...
# whose `path` is exactly the hygiene workflow's: check run -> check_suite.id -> the workflow run with
...
#
# NOT CHECKED: that any particular workflow ran. One green completed check run from any workflow other
# than release-hygiene.yml is enough evidence. See the ADR-0055 addendum (2026-09-12).
#
...
# a name shared by two suites is judged twice, once per suite — never once for both.
report="$(jq -cn --slurpfile runs "$runs_file" --arg hygiene "$HYGIENE_WORKFLOW" "$JQ_DEFS"'
  [ inputs as $c
    | { name: $c.name, status: $c.status, conclusion: $c.conclusion,
        paths: ([ $runs[] | select(.check_suite_id == $c.check_suite.id) | .path ] | unique),
        app: (($c.app | objects | .slug) // null),
        url: (($c.html_url | strings) // null) } ]
  | { pending:  map(select(.status != "completed")),
      excluded: map(select(.status == "completed" and .paths == [$hygiene])),
      judged:   map(select(.status == "completed" and .paths != [$hygiene])) }
...
printf '%s' "$report" | jq -r --arg sha "$sha" --arg hygiene "$HYGIENE_WORKFLOW" "$JQ_DEFS"'
  ...
  | (if (.excluded | length) > 0 then
       "::notice::release-provenance: ignored \(.excluded | length) check run(s) from \($hygiene) — they judge the repository against its tags (a release not cut, a baseline not moved), not whether \($s) was built (ADR-0055 addendum): \(.excluded | map(item) | join(", ") | cmd)"
     else empty end),
    (if (.pending | length) > 0 then
       "release-provenance: not completed, so not evidence either way: \(.pending | map(item) | join(", ") | cmd)"
     else empty end),
    (if .verdict == "no-evidence" then
       "::error::No completed check runs for \($s)\(if (.excluded | length) > 0 then " outside \($hygiene)" else "" end) — nothing proves this commit was ever built."
