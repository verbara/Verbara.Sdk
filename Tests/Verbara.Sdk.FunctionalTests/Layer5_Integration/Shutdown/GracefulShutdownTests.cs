namespace Verbara.Sdk.FunctionalTests.Layer5_Integration.Shutdown;
...
public sealed class GracefulShutdownTests : FunctionalTestBase
{
    ...
    public async Task HostShutdown_ShouldCloseAmiConnection()
    {
        using var host = Host.CreateDefaultBuilder()
            ...
            .ConfigureServices(services => services.AddVerbara(o =>
            {
    ...
    public async Task HostShutdown_ShouldStopAudioSocketServer()
    {
        ...
        var options = new Verbara.Sdk.VoiceAi.AudioSocket.AudioSocketOptions
        {
            Port = 18765,
        ...
        var audioLogger = loggerFactory.CreateLogger<Verbara.Sdk.VoiceAi.AudioSocket.AudioSocketServer>();
        var server = new Verbara.Sdk.VoiceAi.AudioSocket.AudioSocketServer(options, audioLogger);
        ...
        var pendingTasks = Enumerable.Range(0, 10)
            .Select(_ => Task.Run(async () =>
                await connection.SendActionAsync(
                    new Verbara.Sdk.Ami.Actions.PingAction(), actionCts.Token)))
            .ToList();
