# docs-brand-consistency Specification

## Purpose

Keep shipped, living documentation (package and example READMEs) honest against the real public
API surface. Every code identifier a reader could copy-paste MUST name a symbol that exists in the
current codebase — no pre-rebrand `Asterisk*` DI extension methods, options types, or Live-API
types that were renamed to `Verbara*` post-2.0.0, and no fictional API that never existed. Snippets
describing an API with no post-rebrand equivalent are rewritten against an authoritative in-repo
example rather than mechanically token-swapped. Runtime DATA values that only look like residue —
message-bus subject strings and configuration-section binding keys — are explicitly exempt, because
altering them would break wire or configuration contracts. This is the capability the `/xr:doctor`
rebrand-residue sweep enforces.
