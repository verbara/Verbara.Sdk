# claim-guards Specification

## Purpose

Numbers this repo publishes — throughput, latency, accuracy, counts — carry a declared guard class,
so every figure a reader takes as current is either executed by a gate (ENFORCING), checked against
a committed measurement record (COHERENCE), cited as a third party's published figure pinned to the
artifact it describes (ATTRIBUTED), or kept as a dated first-party record of something outside this
repo's control (EVIDENCE). This capability governs where those classes are declared, which guards
may run on a pull request and which stay off that path, how a benchmark baseline moves, and what
happens to a claim nobody can measure in-repo.
