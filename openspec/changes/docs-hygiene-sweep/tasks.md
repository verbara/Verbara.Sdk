...
- [ ] 1.1 Reproduce the doctor sweep and confirm the 13-file inventory:
...
- [ ] 1.2 Verify the replacement symbols exist in real source before any edit:
...
- [ ] 1.3 Confirm the two data-value preserve-classes: the NATS subjects `asterisk.sdk.calls…`
...
- [ ] 2.1 `Examples/NatsBridgeExample/README.md` — `AddAsteriskPush` → `AddVerbaraPush` (line ~38).
      Do NOT touch the `asterisk.sdk.calls…` NATS subject literals (runtime data).
- [ ] 2.2 `Examples/SessionExample/README.md` — `AddAsteriskSessions()` → `AddVerbaraSessions()`,
      `AddAsterisk()` → `AddVerbara()` (lines ~24, 33).
- [ ] 2.3 `Examples/SessionExtensionsExample/README.md` — `AddAsteriskSessions()` →
      `AddVerbaraSessions()`, `AddAsterisk()` → `AddVerbara()` (lines ~27, 35).
- [ ] 2.4 `src/Verbara.Sdk.Ami/README.md` — `AddAsterisk(` → `AddVerbara(` (line ~18).
- [ ] 2.5 `src/Verbara.Sdk.Push/README.md` — `AddAsteriskPush(` → `AddVerbaraPush(` (line ~40).
- [ ] 2.6 `src/Verbara.Sdk.Push.Nats/README.md` — `AddAsteriskPush()` → `AddVerbaraPush()`
      (line ~13).
- [ ] 2.7 `src/Verbara.Sdk.Push.AspNetCore/README.md` — `AddAsteriskPushAspNetCore()` →
      `AddVerbaraPushAspNetCore()` (line ~18).
- [ ] 2.8 `src/Verbara.Sdk.Push.Webhooks/README.md` — `AddAsteriskPush()` → `AddVerbaraPush()`,
      `AddAsteriskPushWebhooks` → `AddVerbaraPushWebhooks` (lines ~11, 12, 45).
- [ ] 2.9 `src/Verbara.Sdk.Resilience/README.md` — `AddAsteriskResilience(` →
      `AddVerbaraResilience(` (line ~30).
- [ ] 2.10 `src/Verbara.Sdk.Sessions/README.md` — `AddAsterisk(` → `AddVerbara(`,
      `AddAsteriskSessions(` → `AddVerbaraSessions(` (lines ~17, 18, 38).
- [ ] 2.11 `src/Verbara.Sdk.Sessions.Redis/README.md` — `AddAsteriskSessionsBuilder()` →
      `AddVerbaraSessionsBuilder()` (lines ~8, 21).
- [ ] 2.12 `src/Verbara.Sdk.Sessions.Postgres/README.md` — `AddAsteriskSessionsBuilder()` →
      `AddVerbaraSessionsBuilder()` (lines ~16, 29).

## 3. Hosting README — identifiers + bare types + fictional-API rewrite

- [ ] 3.1 `src/Verbara.Sdk.Hosting/README.md` DI-call + prose identifiers:
...
- [ ] 3.2 Bare type references (outside the doctor regex, same defect):
...
- [ ] 3.3 **CRITICAL — REWRITE the fictional multi-server snippet (line ~79).**
...
- [ ] 3.4 **PRESERVE** the `"Asterisk"` `appsettings.json` config-section key (line ~36) — it is a
      runtime binding key (`GetSection("Asterisk:Ami")`), NOT an identifier. Leave byte-for-byte.

## 4. Changelog

- [ ] 4.1 Add a `CHANGELOG.md` `## [Unreleased]` entry under `### Changed` (or `### Docs`):
...
- [ ] 5.1 Re-run the 1.1 sweep — zero `AddAsterisk|AsteriskTelemetry|AsteriskSemanticConventions|
...
- [ ] 5.2 Confirm data values survived: `grep -n 'asterisk\.sdk\.calls' Examples/NatsBridgeExample/README.md`
      still present; `"Asterisk"` config key still present in `src/Verbara.Sdk.Hosting/README.md`.
- [ ] 5.3 Confirm the rewritten snippet names only real symbols
      (`AddVerbaraMultiServer`, `VerbaraServerPool`, `AddServerAsync`, `AmiConnectionOptions`) —
      grep each against real source.
- [ ] 5.4 `openspec validate --all --strict` passes. No `dotnet` build/test task applies (docs-only,
