# Product alignment audit — Verbara.Sdk v1.11.0
...
| 0003 | Source generators over reflection | Accepted | [src/Verbara.Sdk.Ami.SourceGenerators/](../../src/Verbara.Sdk.Ami.SourceGenerators/) (4 generators: ActionSerializer, EventDeserializer, EventRegistry, ResponseDeserializer), `[JsonSerializable]` contexts (e.g. `AriJsonContext` in `src/Verbara.Sdk.Ari/Resources/AriJsonContext.cs`), `[OptionsValidator]` partials across every `*OptionsValidator.cs` | None |
...
| 0005 | Testcontainers for integration | Accepted | [Tests/Verbara.Sdk.FunctionalTests/Infrastructure/Fixtures/AsteriskContainerFixture.cs](../../Tests/Verbara.Sdk.FunctionalTests/Infrastructure/Fixtures/AsteriskContainerFixture.cs), `RealtimeFixture.cs`, `ToxiproxyFixture.cs`, [docker/Dockerfile.asterisk](../../docker/Dockerfile.asterisk). Testcontainers bumped to 4.11 (commit `eeb4a2c`) | None |
| 0006 | Pluggable session stores | Accepted | [src/Verbara.Sdk.Sessions/ISessionStore.cs](../../src/Verbara.Sdk.Sessions/ISessionStore.cs) plus `src/Verbara.Sdk.Sessions.Redis/RedisSessionStore.cs` and `src/Verbara.Sdk.Sessions.Postgres/PostgresSessionStore.cs` | None |
| 0007 | Topic hierarchy on Push bus | Accepted | [src/Verbara.Sdk.Push/Topics/TopicPattern.cs](../../src/Verbara.Sdk.Push/Topics/TopicPattern.cs), `TopicName.cs`, `ITopicRegistry.cs`, `TopicRegistry.cs` | None |
| 0008 | AMI exponential backoff reconnect | Accepted | [src/Verbara.Sdk.Ami/Connection/AmiConnection.cs](../../src/Verbara.Sdk.Ami/Connection/AmiConnection.cs) (`ReconnectLoopAsync`), `AmiConnectionOptions.cs` | None |
| 0009 | Three-tier test strategy | Accepted | [Tests/Verbara.Sdk.FunctionalTests/Layer2_UnitProtocol/](../../Tests/Verbara.Sdk.FunctionalTests/Layer2_UnitProtocol/), `Layer5_Integration/`, plus dedicated `Tests/Verbara.Sdk.IntegrationTests/` alongside 21 unit-test projects | None |
| 0010 | ARI asymmetric transport (HTTP out, WS in) | Accepted | [src/Verbara.Sdk.Ari/Client/AriClient.cs](../../src/Verbara.Sdk.Ari/Client/AriClient.cs) uses `HttpClient` for REST + `ClientWebSocket` for events; 10 `AriXxxResource.cs` files in `src/Verbara.Sdk.Ari/Resources/` | None |
| 0011 | Push bus in-memory, non-durable | Accepted | [src/Verbara.Sdk.Push/Bus/RxPushEventBus.cs](../../src/Verbara.Sdk.Push/Bus/RxPushEventBus.cs), `BackpressureStrategy.cs`, `PushEventBusOptions.cs` | None |
| 0012 | Live is an orthogonal aggregate root | Accepted | [src/Verbara.Sdk.Live/](../../src/Verbara.Sdk.Live/) (`AsteriskServer`, `Agents/`, `Bridges/`, `Channels/`, `MeetMe/`, `Queues/`), `ILiveObject.cs` | None |
...
|------|------------------|-----------------------|--------|
| `2026-03-20-functional-testing-phase1-plan.md` | shipped-fully | Implementation at `Tests/Verbara.Sdk.FunctionalTests/`. Task 10 (Pro scaffolding) was redacted in commits `cbb86c2`/`de80e80` — intentional public-repo scrub | OK, no action |
| `2026-03-21-functional-testing-phase5a-plan.md` | shipped-fully | Realtime DB functional tests landed; Testcontainers migration included a PostgreSQL container at `Tests/Verbara.Sdk.FunctionalTests/Infrastructure/Fixtures/RealtimeFixture.cs` and `PostgresSessionStore` integration coverage in `Tests/Verbara.Sdk.IntegrationTests/Sessions/` | OK, no action |
| `2026-03-21-functional-testing-phase5c-plan.md` | shipped-fully | Soak and metrics tests shipped; `SessionsBackendsBenchmark` + `PostgresLatencyBenchmark` are the modern equivalents in `Tests/Verbara.Sdk.Benchmarks/SessionsBackendsBenchmark.cs` | OK, no action |
...
| `2026-03-21-functional-testing-phase2-design.md` | shipped-fully | The 32 source-generator + queue + health-check tests shipped under `Tests/Verbara.Sdk.FunctionalTests/Layer2_UnitProtocol/` | OK, no action |
| `2026-03-21-functional-testing-phase4a-design.md` | shipped-fully | DTMF, IVR, Bridge, Transfer, Parking tests live at `Tests/Verbara.Sdk.FunctionalTests/Layer5_Integration/` | OK, no action |
...
- `find src/Verbara.Sdk.Ami/Actions -name "*.cs" -not -name "*Base*" -not -name "*Registry*" | wc -l` → **148**. The directory holds 149 entries, one of which is the action base wiring; the remaining 148 are concrete actions.
- `ls src/Verbara.Sdk.Ari/Resources/` → 10 resource files plus `AriJsonContext.cs`. Each resource file exposes multiple endpoint methods; the running total lines up with the 94 endpoints reported in the README v1.10.2 block.
...
[2026-03-22-api-completeness-plan.md](../plans/archived/2026-03-22-api-completeness-plan.md) is legitimately archived. Sprints A and B shipped; Sprint C shipped in spirit (AudioSocket was the major deliverable and it is in `src/Verbara.Sdk.VoiceAi.AudioSocket/`; the residual legacy channel-technology AMI actions were explicitly deprioritized in the plan's own "Not Prioritized" list). The 97%/96% coverage number in the README reflects an intentional product scoping decision on modern Asterisk deployments, not an abandoned sprint.
