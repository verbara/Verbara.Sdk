...
| "Docs examples runnable" | Nuevo proyecto `Tests/Verbara.Sdk.DocSnippets.Tests/`: hook MSBuild que extrae bloques \`\`\`csharp de `README.md`, `docs/README-technical.md`, `docs/guides/*.md`; genera `*.g.cs` envuelto en `class Snippet_Filename_LineNN { void M() { … } }`; falla la build si no compila. Esto habría atrapado todo el drift de los helpers de registro pre-rebrand → `AddVerbara*` que arreglamos hoy |
