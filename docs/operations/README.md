...
Prefabricated Grafana dashboards and Jaeger query examples for the SDK's OpenTelemetry catalog (15 meters + 9 `ActivitySource`s — see `VerbaraTelemetry.MeterNames` / `.ActivitySourceNames` in `Verbara.Sdk` for the canonical list). These are **starter templates**, not production-tuned dashboards — copy into your Grafana instance and adapt thresholds / labels / queries to your deployment.
...
| [`jaeger-queries.md`](jaeger-queries.md) | Query patterns for every SDK `ActivitySource` with example tag filters using `VerbaraSemanticConventions`. |
...
```csharp
builder.Services.AddVerbaraOpenTelemetry()
