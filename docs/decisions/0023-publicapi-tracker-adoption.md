...
## Addendum (2026-09-12) — the Shipped file is only as current as the release step; package validation is what compares against a release

The guarantee above — breaking changes surface "as explicit line changes in the Shipped file" — holds for API that has been promoted into `PublicAPI.Shipped.txt`, and the promotion step has not kept pace with releases. At `v2.5.1`, the release commit itself, 20 of the 29 packages still list API in `PublicAPI.Unshipped.txt`, and ten packages that expose API have an empty `PublicAPI.Shipped.txt` although they have been on nuget.org for months. Removing API that is still listed as unshipped is an ordinary line deletion in the Unshipped file — the same diff as removing API that never shipped.

The check that compares against what was actually published is package validation, which `docs/specs/architecture.md` pairs with this tracker. It was switched off for those same ten packages, and for two more, until 2026-09-12. How every shipped package is now validated, and the guard that keeps it so, is recorded in the ADR-0055 addendum "all 29 packed, but only 17 were validated". Promoting the tracker files is not part of that change.
