...
- **`AsteriskSemanticConventions.Events`** nested class — span event names for transient, event-shaped telemetry (use with `Activity.AddEvent`, not `SetTag`). First two entries: `asterisk.channel.hangup` and `asterisk.dtmf.received`. `WebSocketAudioSession` now emits these events on `Activity.Current` when a chan_websocket DTMF or hangup control message arrives. No-op when no span is active. ([df0fe93](https://github.com/Harol-Reina/Asterisk.Sdk/commit/df0fe93))
