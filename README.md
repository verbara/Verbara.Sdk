...
Full methodology, machine-readable results, and cross-language comparison (asterisk-java, asterisk-ami-client, pyst2) in [docs/research/benchmark-analysis.md](docs/research/benchmark-analysis.md). Raw BenchmarkDotNet reports are under `BenchmarkDotNet.Artifacts/results/`. Reproduce: `dotnet run -c Release --project Tests/Verbara.Sdk.Benchmarks/`. **The two session-store rows were measured differently from the header:** re-measured 2026-09-12 on .NET 10.0.12 (same machine model) with an xunit `Fact` + `Stopwatch` against local Docker — PostgreSQL 18.4, Redis 7.4.8 — not BenchmarkDotNet, median of five runs; method, runs and reproduce commands are in that document's addendum. Postgres single-save latency is the measuring machine's WAL flush (one fsync per commit), not SDK code.
