...
- Iterating `VerbaraTelemetry.ActivitySourceNames` and `VerbaraTelemetry.MeterNames` (both `static readonly string[]`) to discover the full telemetry surface at runtime.
...
Because the example does not `AddVerbara(...)` itself, no spans or metrics will be emitted on their own. Add the SDK's DI extensions to see live traffic, or point the example at a separate process emitting the same sources.
...
builder.Services
    .AddOpenTelemetry()
    .WithTracing(t => t
        .AddSource([.. VerbaraTelemetry.ActivitySourceNames])
        .AddConsoleExporter()
        .AddOtlpExporter())              // send to Jaeger, Tempo, etc.
    .WithMetrics(m => m
        .AddMeter([.. VerbaraTelemetry.MeterNames])
        .AddConsoleExporter()
...
Hard-coding the 9 `ActivitySource` names and 15 `Meter` names locks you to a specific SDK version. When future releases add new packages, you would need to update the consumer. Reading from `VerbaraTelemetry.*Names` keeps the consumer forward-compatible — the SDK is the single source of truth for what telemetry it publishes.
